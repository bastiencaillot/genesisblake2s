...
def get_args():
  ...
  parser.add_option("-z", "--timestamp", dest="timestamp", default="CNBC Sep 07 2021 El Salvador bought $21 million of bitcoin as it becomes first country to make it a legal currency",
                   type="string", help="the pszTimestamp found in the coinbase of the genesisblock")
  parser.add_option("-n", "--nonce", dest="nonce", default=0,
                   type="int", help="the first value of the nonce that will be incremented when searching the genesis hash")
  parser.add_option("-a", "--algorithm", dest="algorithm", default="Blake2s",
                    help="the PoW algorithm: [Blake2s]")
...
def generate_hash(data_block, algorithm, start_nonce, bits):
  ...
  while True:
    blake2s_hash, header_hash = generate_hashes_from_block(data_block, algorithm)
    last_updated             = calculate_hashrate(nonce, last_updated)
    if is_genesis_hash(header_hash, target):
      return (blake2s_hash, nonce)
    else:
     nonce      = nonce + 1
...
def generate_hashes_from_block(data_block, algorithm):
  blake2s_hash = hashlib.blake2s(data_block).digest()[::-1]
  header_hash = ""
  header_hash = blake2s_hash
  return blake2s_hash, header_hash
